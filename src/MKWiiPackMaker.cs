...
namespace MKWiiPackMaker
{
    ...
    {
        private const string AppVersion = "12.1.0";
        ...
        private void ShowGettingStartedTutorial(bool firstRun)
        {
            using (Form dialog = new Form())
            {
                ...
                steps.AppendText("\nYou need your own legally dumped Mario Kart Wii disc image or an already extracted MKWii dump. Do not include Nintendo base files when you share the app.\n\n");
                steps.AppendText("FAST METHOD - Let this app extract the required files\n");
                steps.AppendText("1. Go to Setup.\n2. Click Extract Game Image.\n3. Choose your own Mario Kart Wii .iso, .wbfs, .rvz, .gcz, .wia, .wdf, .ciso, or .nkit.iso.\n4. The app uses tools\\wit.exe and copies only the required base folders into base_files.\n\n");
                ...
                warning.Text = "Important: do not distribute Nintendo base_files with the public app. Share the tool with an empty base_files folder. Each user must provide their own MKWii dump or extract base files from their own disc image.";
                ...
                Button extractNow = CreateActionButton("Extract Game Image", AccentGreen, delegate { dialog.Close(); ExtractRequiredBaseFilesFromDiscImage(); });
        ...
        private void ShowReleaseSafetyCheck()
        {
            ...
            string[] riskyExt = new string[] { "*.wbfs", "*.iso", "*.rvz", "*.gcz", "*.wia", "*.wdf", "*.ciso", "*.nkit.iso", "*.szs", "*.brsar", "*.brres" };
            HashSet<string> discImagePatterns = new HashSet<string>(StringComparer.OrdinalIgnoreCase)
            {
                "*.wbfs", "*.iso", "*.rvz", "*.gcz", "*.wia", "*.wdf", "*.ciso", "*.nkit.iso"
            };
            foreach (string pattern in riskyExt)
            {
                ...
                else if (discImagePatterns.Contains(pattern) && count > 0) warn("Found " + count + " disc image file(s) matching " + pattern + ". Never publish game images.");
        ...
        private async void ExtractRequiredBaseFilesFromDiscImage()
        {
            ...
            ofd.Title = "Choose your Mario Kart Wii game image";
            ofd.Filter = "Supported Wii images (*.iso;*.wbfs;*.rvz;*.gcz;*.wia;*.wdf;*.ciso;*.nkit.iso)|*.iso;*.wbfs;*.rvz;*.gcz;*.wia;*.wdf;*.ciso;*.nkit.iso|All files (*.*)|*.*";
            ...
            try
            {
                ...
                if (!ok)
                {
                    ...
                    MessageBox.Show(this,
                        BuildDiscImageExtractionErrorMessage(imagePath, output),
        ...
        private string BuildDiscImageExtractionErrorMessage(string imagePath, string toolOutput)
        {
            string ext = Path.GetExtension(imagePath).ToLowerInvariant();
            string name = Path.GetFileName(imagePath).ToLowerInvariant();
            bool isNkit = name.EndsWith(".nkit.iso", StringComparison.OrdinalIgnoreCase);
            StringBuilder msg = new StringBuilder();
            msg.AppendLine("wit.exe could not extract the selected image.");
            msg.AppendLine();
            msg.AppendLine("Supported selector formats: ISO, WBFS, RVZ, GCZ, WIA, WDF, CISO, and NKit ISO.");
            msg.AppendLine("The image must be a valid Mario Kart Wii dump and your drive must have enough free space.");
            if (ext == ".rvz")
            {
                msg.AppendLine();
                msg.AppendLine("RVZ note: RVZ is a Dolphin disc-image format. If your current wit.exe build cannot read this RVZ, convert the RVZ to ISO or WBFS in Dolphin first, then run extraction again.");
            }
            if (ext == ".gcz" || ext == ".wia" || ext == ".wdf" || ext == ".ciso" || isNkit)
            {
                msg.AppendLine();
                msg.AppendLine("Compressed-image note: if extraction fails, convert the image to ISO or WBFS first, then retry.");
            }
            if (!string.IsNullOrWhiteSpace(toolOutput))
            {
                msg.AppendLine();
                msg.AppendLine("Tool output:");
                msg.AppendLine(toolOutput);
            }
            return msg.ToString();
        }
        ...
        {
            SetActiveNav("Setup");
            SetHeader("Setup", "Extract the required base files from your own MKWii disc image, then build packs normally.");
            ClearPage();
            ...
            Button extractHero = CreateActionButton("Extract Game Image", AccentGreen, delegate { ExtractRequiredBaseFilesFromDiscImage(); });
            ...
            info.Text = "Quick setup: click Extract Game Image and choose your own legally dumped MKWii disc image. Supported inputs include ISO, WBFS, RVZ, GCZ, WIA, WDF, CISO, and NKit ISO. The app copies only the required folders into base_files.";
            ...
            WritePatchLog("Ready. Put extracted MKWii files in ./base_files, or click Extract Game Image to extract them from your own disc image.");
        ...
        private string BuildExportErrorHelpText()
        {
            return "Export error helper\n" +
                   ...
                   "wit failed -> disc image extraction failed; check the image path, format, and free disk space.\n" +
        ...
        private string ExplainExportError(string message)
        {
            ...
            if (m.Contains("wit") || m.Contains("wbfs") || m.Contains("iso") || m.Contains("rvz") || m.Contains("gcz") || m.Contains("wia") || m.Contains("wdf") || m.Contains("ciso") || m.Contains("nkit")) return "Likely cause: disc extraction failed. Check the disc image file, format, and free disk space.";
        ...
        private void ProjectCleanupNow()
        {
            DialogResult confirm = MessageBox.Show(this,
                "Clean safe temporary project files now?\n\nThis removes old working SZS folders, temporary disc image extraction folders, stale auto-generated UI entries, and imported asset rows whose source files no longer exist.\n\nIt will not delete base_files, tools, source files, or exported packs.",
                "Project Cleanup",
